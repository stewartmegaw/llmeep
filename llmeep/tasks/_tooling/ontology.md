...
| `prioritise` | `tm prioritise <id> [-n \| --after <id>]` | `backlog` → `prioritised`, at the bottom (`-n` for the top, `--after <id>` behind one already ranked). **The order of the queue is askable** (`DEC-056`): position *is* priority, so changing it is a decision, and it had no verb until an interface needed one. By neighbour rather than index — an index is a claim about a list somebody else may have just changed. |
