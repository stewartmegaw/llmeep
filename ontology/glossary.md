...
| **task**       | A unit of intended change with an ID. Defined in `taskman/ontology.md`.       |
...
| **record**     | Any managed file under `taskman/` or `notes/`.                                |
...
Taskman has its own avoid-list — see [`taskman/ontology.md`](../taskman/ontology.md).

## Naming conventions

- **Task IDs:** `PLT-9puy`, `BUS-hg7f`. Randomly allocated so branches cannot collide; see
  [`taskman/ontology.md`](../taskman/ontology.md). Never reused, never renumbered.
