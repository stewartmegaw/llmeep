...
`notes/` is a self-contained subsystem on the same terms as `tasks/`: its vocabulary lives
...
| Question | Answer | Why |
|---|---|---|
| Executable? | **`notes/nm`**, separate from `tm` | Keeps both subsystems liftable. `nm promote` shells out to `tasks/_tooling/tm add`. |
